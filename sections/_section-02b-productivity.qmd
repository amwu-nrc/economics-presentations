...
national_accounts <- read_abs(
  cat_no = "5206.0",
  tables = c(1, 7),
  path = "data"
)

productivity_decouple <- read_absdata(
  "productivity",
  export_dir = "data",
  refresh = T
) |>
  pivot_longer(
    cols = c(gdp_per_hour_index, real_compensation_per_hour_index)
  ) |>
  mutate(
    name = case_when(
      name == "gdp_per_hour_index" ~ "Productivity (GDP per Hour Worked)",
      name ==
        "real_compensation_per_hour_index" ~ "Real Compensation per Hour Worked"
    )
  )

productivity_sector <- read_abs_local(
  "5204.0",
  filenames = "5204015_Labour_Productivity_Input.xlsx",
  path = "data"
) |>
  filter(
    table_no == "5204015_Labour_Productivity_Input",
    str_detect(series, "GVA"),
    !is.na(value)
  ) |>
  mutate(
    industry = str_extract(series, "^(.*?);"),
...
::: notes
We need to understand productivity - how it's measured, and how bosses talk about it - because it is simultaneously misunderstood and over-emphasised in the media.

The newly elected government has set its focus on productivity, and productivity growth, and has got the Productivity Commission in on the action.
...
national_accounts_productivity <- national_accounts |>
  ...
  mutate(
    index = 100 * value / value[date == "1991-06-01"],
    series = case_when(
      series == "GDP per hour worked: Index ;" ~ "Labour Productivity",
      series ==
        "Gross value added per hour worked market sector: Index ;" ~ "Labour Productivity (Market)",
      series ==
        "Gross value added per hour worked non-market sector: Index ;" ~ "Labour Productivity (Non-Market)"
    )
  )
...
national_accounts_productivity |>
  ...
  scale_colour_nrc() +
  scale_x_date(
    breaks = plot_date_range(national_accounts_productivity),
    date_labels = "%e %b\n%Y"
  ) +
  labs(
    title = "GDP per Hour Worked",
    subtitle = "Seasonally Adjusted; Quarterly; Index (June 1991 = 100)"
  ) +
...
national_accounts_20 <- national_accounts |>
  filter(
    series %in% c("GDP per hour worked: Index ;"),
    series_type == "Seasonally Adjusted"
  ) |>
...
national_accounts_20 |>
  ...
  scale_x_date(
    breaks = plot_date_range(national_accounts_20),
    date_labels = "%e %b\n%Y"
  ) +
  labs(
    title = "Declining Productivity Growth",
    subtitle = "20-Year Average Quarterly Productivity Growth",
    x = NULL,
    y = NULL
  ) +
  theme_nrc()
...
national_accounts_hw <- national_accounts |>
  ...
  mutate(
    series = case_when(
      ...
      series ==
        "Hours worked non-market sector: Index ;" ~ "Hours Worked (Non-Market)"
    )
  )
...
national_accounts_hw |>
  ...
  labs(
    ...
    y = NULL,
    title = "Hours Worked",
    subtitle = "Seasonally Adjusted; Quarterly; Index: (June 2022 = 100)"
  )
```
...
national_accounts_gdp |>
  ...
  scale_y_continuous(
    labels = scales::label_currency(scale = 1e-3, suffix = "bn")
  )
```
...
productivity_sector |>
  ...
  scale_colour_nrc() +
  gghighlight(
    industry %in% c("ALL INDUSTRIES", "Manufacturing", "Mining"),
    line_label_type = "ggrepel_text"
  ) +
  theme_nrc() +
  labs(
    title = "Industry Productivity",
    subtitle = "Gross Value Added per Hour Worked (June 1998 = 100)",
    x = NULL,
    y = NULL
  )
```
